...
namespace ui {
	...
	void print_rules(int field_lines) {
		...
		int right_indent = get_terminal_width() - 68;
	...
	void print_centered(const std::wstring& text) {
		...
		std::wcout << L"\x1b[" << get_terminal_height() << L"A";
		print_separator_half();
	...
	Synopsis: Prints a horizontal separator line. (full-width)
	---------------------------------------------------------------------[>]*/
